        ...
        $client = new Client([
            ...
            foreach ($comments as $rating) {
                if ($rating->movie_id == $id) {
                    array_push($ratings, $rating->star);
                }
            }
            if (count($ratings)) {
            ...
            return view('movie', ['movie' => $movie,'crew'=>$crew['cast'],'director'=>$crew['crew'],'trailer'=>$trailer['results'], 'comment' => $comment['results'], 'recommendations'=> $recommendations['results'], 'images'=>$images['backdrops'] ,'comments'=>$comments, 'critics' => $critics,'average' => $average]);
        }
    }
